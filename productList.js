...
search.addEventListener("click", () => {
  ...
  if (check) {
    document.querySelector(
      "#searchBar"
    ).innerHTML += `<input type="text" style="margin:10px;" class="form-control mb-2" placeholder="Enter Your Shirt" id="form">`;
    ...
    log.addEventListener("keyup", () => {
      ...
      let infoOfProduct = divID.getElementsByClassName("col-3");
...
function listProduct() {
  for (let i = 0; i < product.length; i++) {
    ...
    imgShein.innerHTML += `<img src='${product[i].productImg}' width="250px" style="marginBottom:20px;"><div class="overlayText"><a href ='#' id="Add ${product[i].productId}"class="textImage">Add</a></div>`;
...
let addCart = document.querySelectorAll(".textImage");
